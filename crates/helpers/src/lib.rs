...
//! Helper utilities for the Veil toolchain.
//!
...
/// Format a parse error diagnostic into a colored, user-friendly string.
...
/// Print a formatted parse error to stderr.
...
/// Validate that the provided path points to an existing .veil file.
...
/// Discover a suitable clang path for the current platform.
/// Honors environment variables for configurability:
/// - VEIL_CLANG_PATH (absolute path override)
/// - VEIL_USE_SYSTEM_CLANG=1 (prefer PATH 'clang')
...
/// Prepare platform-appropriate clang arguments for Windows.
